...
#warning "<enumivolib/binary_extension.hpp> is deprecated use <enumivo/binary_extension.hpp>"
 ...
 namespace enumivo {
   ...
   class binary_extension {
      public:
         ...
         constexpr T& value()& {
            if (!_has_value) {
               enumivo_assert(false, "cannot get value of empty binary_extension");
            }
            return _get();
         }
          /** get the contained value */
         ...
         **/
         ...
         friend inline DataStream& operator<<(DataStream& ds, const enumivo::binary_extension<T>& be) {
            ds << be.value_or();
            return ds;
         }
         ...
         /**
           ...
           */
         template<typename DataStream>
         friend inline DataStream& operator>>(DataStream& ds, enumivo::binary_extension<T>& be) {
            if( ds.remaining() ) {
               T val;
               ds >> val;
               be.emplace(val);
