...
#warning "<enumivolib/transaction.hpp> is deprecated use <enumivo/transaction.hpp>"
namespace enumivo {
   ...
   */
   ...
   public:
      ...
      ENULIB_SERIALIZE( transaction_header, (expiration)(ref_block_num)(ref_block_prefix)(max_net_usage_words)(max_cpu_usage_ms)(delay_sec) )
   };
   ...
   inline action get_action( uint32_t type, uint32_t index ) {
      ...
      enumivo::check( s > 0, "get_action size failed" );
      size_t size = static_cast<size_t>(s);
      char* buffer = (char*)( max_stack_buffer_size < size ? malloc(size) : alloca(size) );
      auto size2 = ::get_action( type, index, buffer, size );
      enumivo::check( size == static_cast<size_t>(size2), "get_action failed" );
