...
namespace enumivo {
   ...
   */
   ...
   public:
      ...
      {
         if( str.size() > 7 ) {
            enumivo::check( false, "string is too long to be a valid symbol_code" );
         }
         for( auto itr = str.rbegin(); itr != str.rend(); ++itr ) {
            if( *itr < 'A' || *itr > 'Z') {
               enumivo::check( false, "only uppercase letters allowed in symbol_code string" );
            }
            value <<= 8;
            value |= *itr;
         }
   ...
   inline DataStream& operator<<(DataStream& ds, const enumivo::symbol_code sym_code) {
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, enumivo::symbol_code& sym_code) {
     uint64_t raw = 0;
   ...
   public:
      ...
      void print( bool show_precision = true )const {
         if( show_precision ){
            ::enumivo::print( static_cast<uint64_t>(precision()), "," );
         }
         char buffer[7];
         auto end = code().write_as_string( buffer, buffer + sizeof(buffer) );
         if( buffer < end )
            ::enumivo::print( buffer, (end-buffer) );
      }
    ...
    */
   template<typename DataStream>
   inline DataStream& operator<<(DataStream& ds, const enumivo::symbol sym) {
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, enumivo::symbol& sym) {
     uint64_t raw = 0;
     ds.read((char*)&raw, sizeof(raw));
     sym = symbol(raw);
     return ds;
   ...
   private:
      ...
      ENULIB_SERIALIZE( extended_symbol, (symbol)(contract) )
   };
}
