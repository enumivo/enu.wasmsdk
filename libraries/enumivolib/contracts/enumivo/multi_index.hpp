...
#include "../../contracts/enumivo/action.hpp"
#include "../../core/enumivo/name.hpp"
#include "../../core/enumivo/serialize.hpp"
#include "../../core/enumivo/fixed_bytes.hpp"
 ...
 *      }
 *  }
 *  ENUMIVO_DISPATCH( mycontract, (myaction) )
 *  @endcode
 */
 ...
 *  @details Enumivo Multi-Index API provides a C++ interface to the Enumivo database. It is patterned after Boost Multi Index Container.
...
class multi_index
{
   private:
      ...
      struct index {
         public:
            ...
            constexpr static bool validate_index_name( enumivo::name n ) {
               return n.value != 0 && n != enumivo::name("primary"); // Primary is a reserve index name.
            }
            ...
            struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
               public:
                  ...
                  const_iterator& operator++() {
                     ...
                     enumivo::check( _item != nullptr, "cannot increment end iterator" );
                  ...
                  const_iterator& operator--() {
                     ...
                     if( !_item ) {
                        ...
                        enumivo::check( ei != -1, "cannot decrement end iterator when the index is empty" );
                        prev_itr = secondary_index_db_functions<secondary_key_type>::db_idx_previous( ei , &prev_pk );
                        enumivo::check( prev_itr >= 0, "cannot decrement end iterator when the index is empty" );
                     } else {
                        if( _item->__iters[Number] == -1 ) {
                           secondary_key_type temp_secondary_key;
                           auto idxitr = secondary_index_db_functions<secondary_key_type>::db_idx_find_primary(_idx->get_code().value, _idx->get_scope(), _idx->name(), _item->primary_key(), temp_secondary_key);
                        ...
                        enumivo::check( prev_itr >= 0, "cannot decrement iterator at beginning of index" );
                     }
            ...
            const_iterator require_find( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               auto lb = lower_bound( secondary );
               enumivo::check( lb != cend(), error_msg );
               enumivo::check( secondary == secondary_extractor_type()(*lb), error_msg );
               return lb;
            }
            ...
            const T& get( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               ...
               enumivo::check( result != cend(), error_msg );
               return *result;
            }
            ...
            const_iterator iterator_to( const T& obj ) {
               ...
               enumivo::check( objitem.__idx == _multidx, "object passed to iterator_to is not in multi_index" );
            ...
            void modify( const_iterator itr, enumivo::name payer, Lambda&& updater ) {
               enumivo::check( itr != cend(), "cannot pass end iterator to modify" );

               _multidx->modify( *itr, payer, std::forward<Lambda&&>(updater) );
            }

            const_iterator erase( const_iterator itr ) {
               enumivo::check( itr != cend(), "cannot pass end iterator to erase" );
            ...
            enumivo::name get_code()const  { return _multidx->get_code(); }
      ...
      static constexpr auto transform_indices( ) {
         ...
         return hana::transform( indices_input_type(), [&]( auto&& idx ){
             ...
             return hana::make_tuple( hana::type_c<index<enumivo::name::raw(static_cast<uint64_t>(idx_type::index_name)),
                                                         typename idx_type::secondary_extractor_type,
                                                         num_type::e::value, false> >,
                                      hana::type_c<index<enumivo::name::raw(static_cast<uint64_t>(idx_type::index_name)),
                                                         typename idx_type::secondary_extractor_type,
      ...
      const item& load_object_by_primary_iterator( int32_t itr )const {
         ...
         enumivo::check( size >= 0, "error reading iterator" );
   ...
   public:
       ...
       *
       *  Notes
       *  The `enumivo::multi_index` template has template parameters `<name::raw TableName, typename T, typename... Indices>`, where:
       *  - `TableName` is the name of the table, maximum 12 characters long, characters in the name from the set of lowercase letters, digits 1 to 5, and the "." (period) character and is converted to a enumivo::raw - which wraps uint64_t;
      ...
      struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
         ...
         const_iterator& operator--() {
            ...
            if( !_item ) {
               ...
               enumivo::check( ei != -1, "cannot decrement end iterator when the table is empty" );
               prev_itr = internal_use_do_not_use::db_previous_i64( ei , &prev_pk );
               enumivo::check( prev_itr >= 0, "cannot decrement end iterator when the table is empty" );
            } else {
               prev_itr = internal_use_do_not_use::db_previous_i64( _item->__primary_itr, &prev_pk );
               enumivo::check( prev_itr >= 0, "cannot decrement iterator at beginning of table" );
            }
      ...
      uint64_t available_primary_key()const {
         ...
         enumivo::check( _next_primary_key < no_available_primary_key, "next primary key in table is at autoincrement limit");
      ...
      const_iterator iterator_to( const T& obj )const {
         const auto& objitem = static_cast<const item&>(obj);
         enumivo::check( objitem.__idx == this, "object passed to iterator_to is not in multi_index" );
         return {this, &objitem};
      }
      /**
      ...
      const_iterator emplace( name payer, Lambda&& constructor ) {
         ...
         enumivo::check( _code == current_receiver(), "cannot create objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
      ...
      void modify( const_iterator itr, name payer, Lambda&& updater ) {
         enumivo::check( itr != end(), "cannot pass end iterator to modify" );
       ...
       *          address.state = "CA";
       *        });
       *        enumivo::check(itr->city == "San Luis Obispo", "Lock arf, Address not modified");
       *      }
       *  }
       *  ENUMIVO_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      void modify( const T& obj, name payer, Lambda&& updater ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         enumivo::check( objitem.__idx == this, "object passed to modify is not in multi_index" );
         auto& mutableitem = const_cast<item&>(objitem);
         enumivo::check( _code == current_receiver(), "cannot modify objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         enumivo::check( pk == obj.primary_key(), "updater cannot change primary key when modifying an object" );
       ...
       */
      const_iterator erase( const_iterator itr ) {
         enumivo::check( itr != end(), "cannot pass end iterator to erase" );
      ...
      void erase( const T& obj ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         enumivo::check( objitem.__idx == this, "object passed to erase is not in multi_index" );
         enumivo::check( _code == current_receiver(), "cannot erase objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         enumivo::check( itr2 != _items_vector.rend(), "attempt to remove object that was not in multi_index" );
