   ...
   public:
      ...
      */
      inline bool read( char* d, size_t s ) {
        enumivo::check( size_t(_end - _pos) >= (size_t)s, "read" );
        memcpy( d, _pos, s );
        _pos += s;
        return true;
      }
      ...
      */
      inline bool write( const char* d, size_t s ) {
        enumivo::check( _end - _pos >= (int32_t)s, "write" );
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const enumivo::binary_extension<T>& be) {
  ds << be.value_or();
  return ds;
}
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, enumivo::binary_extension<T>& be) {
  if( ds.remaining() ) {
     T val;
     ds >> val;
     be.emplace(val);
...
void deserialize(datastream<Stream>& ds, std::variant<Ts...>& var, int i) {
   if constexpr (I < std::variant_size_v<std::variant<Ts...>>) {
      ...
      enumivo::check(false, "invalid variant index");
   }
}
...
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const enumivo::symbol_code sym_code) {
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, enumivo::symbol_code& sym_code) {
  uint64_t raw = 0;
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const enumivo::symbol sym) {
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, enumivo::symbol& sym) {
  uint64_t raw = 0;
  ds.read((char*)&raw, sizeof(raw));
  sym = symbol(raw);
  return ds;
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const ::enumivo::ignore_wrapper<T>& val) {
  ds << val.value;
  return ds;
}
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const ::enumivo::ignore<T>& val) {
  return ds;
}
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, ::enumivo::ignore<T>) {
  return ds;
}
...
/**
 *  Serialize an enumivo::public_key into a stream
 *
 *  @brief Serialize an enumivo::public_key
 *  @param ds - The stream to write
 *  @param pubkey - The value to serialize
 *  @tparam Stream - Type of datastream buffer
 *  @return datastream<Stream>& - Reference to the datastream
 */
template<typename Stream>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const enumivo::public_key& pubkey) {
   ds << pubkey.type;
...
/**
 *  Deserialize an enumivo::public_key from a stream
 *
 *  @brief Deserialize an enumivo::public_key
 *  @param ds - The stream to read
 *  @param pubkey - The destination for deserialized value
 *  @tparam Stream - Type of datastream buffer
 *  @return datastream<Stream>& - Reference to the datastream
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, enumivo::public_key& pubkey) {
   ds >> pubkey.type;
...
/**
 *  Serialize an enumivo::signature into a stream
 *
 *  @brief Serialize an enumivo::signature
 *  @param ds - The stream to write
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const enumivo::signature& sig) {
   ds << sig.type;
...
/**
 *  Deserialize an enumivo::signature from a stream
 *
 *  @brief Deserialize an enumivo::signature
 *  @param ds - The stream to read
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, enumivo::signature& sig) {
   ds >> sig.type;
...
DataStream& operator >> ( DataStream& ds, T (&v)[N] ) {
   unsigned_int s;
   ds >> s;
   enumivo::check( N == s.value, "T[] size and unpacked size don't match");
   for( uint32_t i = 0; i < N; ++i )
      ds >> v[i];
   return ds;
}
