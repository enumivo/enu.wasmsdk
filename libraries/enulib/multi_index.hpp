...
class multi_index
{
   ...
   public:
       ...
       *      typedef enumivo::multi_index< name("address"), address > address_index;
       *      void myaction() {
       *        address_index addresses(_self, _self.value);  // code, scope
       *        // add to table, first argument is account to bill for storage
      ...
      const_iterator emplace( name payer, Lambda&& constructor ) {
         ...
         enumivo_assert( _code.value == current_receiver(), "cannot create objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
       ...
       *          address.state = "CA";
       *        });
       *        enumivo_assert(itr->city == "San Luis Obispo", "Lock arf, Address not modified");
       *      }
       *  }
       *  ENUMIVO_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      void modify( const T& obj, name payer, Lambda&& updater ) {
         ...
         enumivo_assert( _code.value == current_receiver(), "cannot modify objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
      ...
      void erase( const T& obj ) {
         ...
         enumivo_assert( _code.value == current_receiver(), "cannot erase objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
