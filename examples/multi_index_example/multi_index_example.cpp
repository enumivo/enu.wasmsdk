...
//class [[enumivo::contract]] multi_index_example : public contract {
/* or this method of declaration if you don't want to use the c++ class name */
//class [[enumivo::contract("<some contract name>")]] multi_index_example : public contract {
...
//      void print( name user ) {
      ACTION print( name user ) {
         auto itr = testtab.find(user.value);
         enumivo_assert( itr != testtab.end(), "test table not set" );
         enumivo::print_f("Test Table : {%, %, %}\n", itr->test_primary, itr->secondary, itr->datum);
      }
...
//      struct [[enumivo::table]] test_table {
      TABLE test_table {
         name test_primary;
         name secondary;
         uint64_t datum;
...
ENUMIVO_DISPATCH( multi_index_example, (set)(print)(mod)(bysec) )
