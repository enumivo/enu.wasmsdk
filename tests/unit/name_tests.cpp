...
// Definitions in `enumivo.cdt/libraries/enumivo/name.hpp`
ENUMIVO_TEST_BEGIN(name_type_test)
   silence_output(true);
   ...
   CHECK_EQUAL( name{"enu"}.length(), 3 )
   CHECK_EQUAL( name{"enum"}.length(), 4 )
   CHECK_EQUAL( name{"enumi"}.length(), 5 )
   CHECK_EQUAL( name{"enumiv"}.length(), 6 )
   CHECK_EQUAL( name{"enumivo"}.length(), 7 )
   CHECK_EQUAL( name{"enumivoa"}.length(), 8 )
   CHECK_EQUAL( name{"enumivoac"}.length(), 9 )
